...
> **Via the Viva Direct connector the people table is called `PeopleHistorical` (or `HR`) — not
> `PeopleMetaData`.** That last name is what the CSV export uses and it does **not** work against the
> connector; nor does any `Data_` prefixed variant. Verified live. The join key is
> `PeopleHistoricalId`, which the metrics rows also carry.
